...
// templates/docker-v1.tmpl (7.334kB)
// templates/docker.tmpl (10.248kB)
...
{{range $backendName, $backend := .Backends }}
  ...
  [backends."backend-{{ $backendName }}".loadbalancer]
    ...
    [backends."backend-{{ $backendName }}".loadbalancer.stickiness]
      ...
      headerName = "{{ getStickinessHeaderName $backend }}"
    {{end}}
  {{end}}
...
func templatesDockerV1Tmpl() (*asset, error) {
	...
	a := &asset{bytes: bytes, info: info, digest: [32]uint8{0xdd, 0x6e, 0x3f, 0x90, 0x83, 0x35, 0xfb, 0x8, 0xbf, 0x13, 0xa, 0x66, 0x39, 0xf, 0x43, 0x9e, 0x17, 0x4, 0x88, 0xd3, 0xe5, 0x4d, 0xa, 0x8e, 0xee, 0xce, 0x43, 0xcd, 0x29, 0x9b, 0x8e, 0xb1}}
	return a, nil
}
...
{{ $backend := index $servers 0 }}
  ...
  {{if $loadBalancer }}
    [backends."backend-{{ $backendName }}".loadBalancer]
      ...
      [backends."backend-{{ $backendName }}".loadBalancer.stickiness]
        cookieName = "{{ $loadBalancer.Stickiness.CookieName }}"
        headerName = "{{ $loadBalancer.Stickiness.HeaderName }}"
        secure = {{ $loadBalancer.Stickiness.Secure }}
        httpOnly = {{ $loadBalancer.Stickiness.HTTPOnly }}
        sameSite = "{{ $loadBalancer.Stickiness.SameSite }}"
      {{end}}
...
func templatesDockerTmpl() (*asset, error) {
	...
	a := &asset{bytes: bytes, info: info, digest: [32]uint8{0xe2, 0x8d, 0x4b, 0xf, 0xc2, 0xc0, 0x10, 0xa0, 0x53, 0xc7, 0x63, 0x86, 0x72, 0x46, 0x27, 0x12, 0xa7, 0x48, 0xc2, 0x7, 0x9f, 0x92, 0x5f, 0x1e, 0xae, 0xda, 0xf2, 0x60, 0x91, 0xa, 0x2, 0x9a}}
	return a, nil
}
